...
import { Search, Plus, Copy, Loader2, X, Zap } from "lucide-react"
import type { Order } from "@/components/orders-view"
import { createPrescription } from "@/lib/actions/prescriptions"
import { useToast } from "@/components/ui/use-toast"
import { Badge } from "@/components/ui/badge"
import { cn } from "@/lib/utils"
import { getMedicationGuide, searchMedications } from "@/lib/medication-database"
...
export function PrescriptionsView({ prescriptions = [], orders = [], patientId }: PrescriptionsViewProps) {
  ...
  const medicationSuggestions = useMemo(() => {
    return medicationInputValue.length > 0 ? searchMedications(medicationInputValue) : []
  }, [medicationInputValue])

  const handleMedicationSelect = (medicationName: string) => {
    const guide = getMedicationGuide(medicationName)
    
    if (guide) {
      // Auto-populate all fields from medication guide
      setNewRx((prev) => ({
        ...prev,
        medication: guide.name,
        dosage: guide.defaultDosage.dosage,
        dosageUnit: guide.defaultDosage.dosageUnit,
        dosageForm: guide.defaultDosage.dosageForm,
        frequency: guide.defaultDosage.frequency,
        route: guide.route,
        quantity: guide.standardQuantity.toString(),
        duration: guide.standardDuration.toString(),
        refills: guide.defaultRefills.toString(),
        selectedInstructions: guide.instructionTags,
      }))
      
      toast({
        title: "Medication Guide Loaded",
        description: `${guide.name} - Standard dosing applied. ${guide.warnings.length > 0 ? 'Check warnings.' : ''}`,
      })
    } else {
      // If not in database, just set medication name
      setNewRx((prev) => ({
        ...prev,
          ...
          <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="medication">Medication <span className="text-xs text-amber-600 font-semibold ml-1">⚡ Smart Select</span></Label>
                <div className="relative">
                  <Input
                    id="medication"
                    placeholder="Start typing medication name... (e.g. Amoxicillin, Metformin)"
                    value={medicationInputValue || newRx.medication}
                    onChange={(e) => {
                      setMedicationInputValue(e.target.value)
                      setMedicationSearchOpen(e.target.value.length > 0)
                    }}
                    onFocus={() => setMedicationSearchOpen(medicationInputValue.length > 0)}
                    className="pr-9"
                  />
                  {medicationInputValue && (
                    <button
                  ...
                  {medicationSearchOpen && medicationSuggestions.length > 0 && (
                    <div className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-900 border rounded-lg shadow-xl max-h-64 overflow-y-auto">
                      {medicationSuggestions.map((med) => (
                        <button
                          key={med.id}
                          type="button"
                          onClick={() => handleMedicationSelect(med.name)}
                          className="w-full px-4 py-3 text-left hover:bg-slate-100 dark:hover:bg-slate-800 border-b last:border-0 transition-colors"
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="text-xs text-muted-foreground mt-0.5">{med.category}</div>
                              <div className="text-xs text-slate-600 dark:text-slate-400 mt-1">
                                {med.defaultDosage.dosage}{med.defaultDosage.dosageUnit} • {med.defaultDosage.frequency}
                              </div>
                            </div>
                            <Badge variant="secondary" className="ml-2 text-xs h-fit">Auto-fill</Badge>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {medicationInputValue && medicationSuggestions.length === 0 && medicationSearchOpen && (
                    <div className="absolute z-50 w-full mt-1 bg-white dark:bg-slate-900 border rounded-lg shadow-xl p-4 text-center text-sm text-muted-foreground">
                      No medications found. You can still create a custom prescription.
                    </div>
                  )}
                </div>

                {/* Show loaded medication guide info */}
                {newRx.medication && (
                  <div className="text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-3 py-2 rounded flex items-start gap-2">
                    <Zap className="h-3.5 w-3.5 mt-0.5 flex-shrink-0" />
                    <span>
                      <strong>{newRx.medication}</strong> - Standard dosing loaded from clinical guidelines. Review all fields before prescribing.
                    </span>
                  </div>
                )}
              </div>
              ...
              {newRx.medication && (() => {
                const guide = getMedicationGuide(newRx.medication)
                return guide && guide.warnings.length > 0 ? (
                  <div className="p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-lg space-y-1">
                    <div className="text-xs font-bold text-red-700 dark:text-red-400 uppercase flex items-center gap-1.5">
                      ⚠️ Clinical Warnings & Precautions
                    </div>
